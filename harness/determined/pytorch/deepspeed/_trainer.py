...
from determined import core, gpu, layers, pytorch, util, workload
...
class Trainer:
    """
    ``deepspeed.Trainer`` is an abstraction on top of a vanilla PyTorch training loop that handles
    ...
    ``Trainer`` must be initialized and called from within a ``pytorch.DeepSpeedTrialContext``.
    """

    def __init__(self, trial: det_ds.DeepSpeedTrial, context: det_ds.DeepSpeedContext):
        self._trial = trial
...
) -> Iterator[det_ds.DeepSpeedContext]:
    """
    Creates a DeepSpeedTrialContext for use with a DeepSpeedTrial. All trainer.* calls must be within
